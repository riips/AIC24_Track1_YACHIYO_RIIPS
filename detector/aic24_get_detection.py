...
def make_parser():
    ...
    parser.add_argument("-f", "--exp_file", default="yolox/exps/example/mot/yolox_x_mix_det.py", type=str, help="pls input your expriment description file")
    parser.add_argument("-c", "--ckpt", default="bytetrack_x_mot17.pth.tar", type=str, help="ckpt for eval")
    ...
    for cam in cameras:
        ...
        for frame_id, img_path in enumerate(imgs, 1):
            ...
            for det in detections:
                ...
                x2 = min(1920,x2)
                y2 = min(1080,y2)
                results.append([cam,frame_id,1,int(x1),int(y1),int(x2),int(y2),score])
                det_json = {}
